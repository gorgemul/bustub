...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
  ...
  std::shared_lock reader_lock(frame_->rwlatch_);
...
void ReadPageGuard::Flush() {
  ...
  DiskRequest request{true, frame_->data_.data(), page_id_, std::move(promise)};
  std::vector<DiskRequest> requests;
  requests.push_back(std::move(request));
  disk_scheduler_->Schedule(requests);
  bool success = future.get();
  if (!success) {
    log_die("[WritePageGuard::Flush]: flush fail");
  }
...
void ReadPageGuard::Drop() {
  ...
  bool is_last = frame_->pin_count_.fetch_sub(1) == 1;
  if (is_last) {
...
WritePageGuard::WritePageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
    ...
    : page_id_(page_id),
      ...
      bpm_latch_(std::move(bpm_latch)),
      disk_scheduler_(std::move(disk_scheduler)) {
  std::unique_lock writer_lock(frame_->rwlatch_);
  replacer_->RecordAccess(frame_->frame_id_);
  frame_->pin_count_.fetch_add(1);
  is_valid_ = true;
}
